...
namespace CloudClient
	{
			...
			private int sendAliveMsgToMaster(uint robot_id,uint partition_id)
		{
			...
			newPacket.rs_msg.robot_id =  robot_id;
			newPacket.rs_msg.partition_id = partition_id;
			...
			//Thread.Sleep (100);
				...
				using (var game = new GameWindow())
	            {
	                ...
	                game.RenderFrame += (sender, e) =>
	                {
	                    ...
	                    drawThisMaze(partition_index,game,mainBlockList,stm);
					tcpclient.Close ();
						game.Exit();
...
//			}
			...
			public void drawThisMaze (uint partition_index,GameWindow game,List<MazeBlock> partitions,NetworkStream stm)
			{
				...
				//int index = getRandomIndex (blocks);

//				while (blocks [index].isrendered == true) {
//					index = getRandomIndex (blocks);
//					//game.Exit ();
//				}
			//block_index--;
			...
			foreach(Points p in partition.get_list_of_Points()){
				...
				send_back_this_point (stm, p,r_id,p_id);		
				GL.Begin(PrimitiveType.Points);
		...
		void send_back_this_point (NetworkStream stm, Points p,uint robot_id,uint partition_id)
		{
